(function (global) {
    ...
    function buildPinDescription(meta, point, blockIdx, pointIdx) {
        ...
        if (area) {
            textLines.push('Location: ' + area);
        } else if (latLng) {
            textLines.push('Location: (Click to view)');
        }
        ...
        if (area) {
            ...
            var bIdx = blockIdx !== undefined ? blockIdx : -1;
            var pIdx = pointIdx !== undefined ? pointIdx : -1;
            html += '<div class="circuit-pin-line"><span class="circuit-pin-label">Location:</span> ' +
                '<span class="async-location" data-lat="' + point.lat + '" data-lng="' + point.lng + '" data-bidx="' + bIdx + '" data-pidx="' + pIdx + '">Fetching location...</span></div>';
        }
        if (latLng) {
    ...
    function renderBlocks(blocks) {
        ...
        blocks.forEach(function (block, idx) {
            ...
            block.points.forEach(function (p, pIdx) {
                var desc = buildPinDescription(block.meta, p, idx, pIdx);
    ...
    function initMap() {
        ...
        map.on('popupopen', function(e) {
            ...
            if (!popupNode) return;
            var asyncLocs = popupNode.querySelectorAll('.async-location:not(.fetched)');
            asyncLocs.forEach(function(span) {
                var lat = span.getAttribute('data-lat');
                var lng = span.getAttribute('data-lng');
                var bIdx = parseInt(span.getAttribute('data-bidx'), 10);
                var pIdx = parseInt(span.getAttribute('data-pidx'), 10);

                if (lat && lng) {
                    span.classList.add('fetched');
                    span.innerText = 'Fetching location...';
                    fetch('https://nominatim.openstreetmap.org/reverse?format=json&lat=' + lat + '&lon=' + lng)
                        .then(function(r) { return r.json(); })
                        .then(function(data) {
                            if (data && data.display_name) {
                                span.innerText = data.display_name;
                                if (bIdx >= 0 && pIdx >= 0 && mapState.blocks[bIdx] && mapState.blocks[bIdx].points[pIdx]) {
                                    ...
                                    if (e.popup && e.popup._source) {
                                        var newDesc = buildPinDescription(mapState.blocks[bIdx].meta, p, bIdx, pIdx);
                                        e.popup._source.setTooltipContent(newDesc.text);
                                    }
                                }
                            } else {
                        ...
                        .catch(function() {
                            span.innerText = 'Failed to load location';
                            span.classList.remove('fetched');
                        });
                }
            });
        });
